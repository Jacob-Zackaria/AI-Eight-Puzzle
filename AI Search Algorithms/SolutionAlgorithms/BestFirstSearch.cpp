...
bool BestFS::Solve(std::deque<Node*>& solutionList, int& totalCost, unsigned int& totalTime, unsigned int& totalSpace, int* goalState)
{
	// clear visited list
	visitedList.clear();
		...
		while (childNode != nullptr)
		{
			...
			if (!this->FindInQueue(currentQueue, childNode->GetCurrentState()))
			{
				...
				if (!this->FindInVisited(childNode->GetCurrentState()))
				{
					...
					// sort the queue based on h1 = number of tiles misplaced.
